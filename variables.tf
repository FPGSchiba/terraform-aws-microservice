...
variable "control_allow_methods" {
  ...
  validation {
    condition = var.control_allow_methods == null || (
      ...
      alltrue([for method in var.control_allow_methods : contains(["GET", "POST", "PUT", "DELETE", "HEAD", "PATCH", "TRACE"], method)])
    )
    error_message = "control_allow_methods must be null or a non-empty list containing only the following HTTP Methods: ('GET', 'POST', 'PUT', 'DELETE', 'HEAD', 'PATCH', 'TRACE'). Please read the documentation in order to understand this better."
  }
}
